...
class DatabaseTemplateLoader(BaseLoader):
    """Jinja2 template loader that loads from database."""
    ...
    def get_source(self, environment, template):
        ...
        try:
            ...
            tmpl = EmailTemplate.objects.get(
                slug=template,
                is_active=True
            )
            
            source = tmpl.html_content
...
class EmailService:
    ...
    async def _get_template(self, template_id: str) -> EmailTemplate:
        ...
        try:
            # In production, templates would be filtered by the user's group in the view layer
            # For now, we'll just get the first active template with this slug
            db_template = await sync_to_async(
                lambda: DbEmailTemplate.objects.filter(
                    slug=template_id,
                    is_active=True
                ).first()
            )()
            ...
            template = EmailTemplate(
                ...
                from_email=db_template.from_email,
                from_name=db_template.from_name,
                reply_to=db_template.reply_to_email
            )
